...
<body class="hold-transition skin-red sidebar-mini">
  ...
  $results = $conn->query("SELECT pd.`PID`, pd.`ProductCode`, pd.`Description`, pd.`UnitOfMeasure`, pd.`UnitCost`,
    (SELECT er.QUANTITY  from tblemergencyroom as er WHERE er.PID = pd.PID ) as 'ER',
    (SELECT ph.QUANTITY  from tblpharmacy as ph WHERE ph.PID = pd.PID ) as 'PH',
    (SELECT op.QUANTITY  from tbloperatingroom as op WHERE op.PID = pd.PID ) as 'OP',
    (SELECT wh.QUANTITY  from tblwarehouse as wh WHERE wh.PID = pd.PID ) as 'WH'
    FROM `tblproducts` as pd
    ");
  $no_of_products = $conn->query("SELECT COUNT(*) as 'NoOfProducts' FROM tblproducts ")->fetch();
  ...
  if(isset($_POST["addprod"])){
    ...
    $stmt = $conn->prepare("SELECT `PID` FROM `tblproducts` WHERE `ProductCode`=:pcode AND `Description`=:dscr");
    $stmt->bindParam("pcode", $prod_code) ;
    $stmt->bindParam("dscr", $desc) ;
    $stmt->execute();
    ...
    {
      try{
      $stmt = $conn->prepare("UPDATE `tblwarehouse` SET `QUANTITY`=`QUANTITY` + :additional WHERE `PID`=:pid");
      $stmt->bindParam(':additional', $quantity);
      $stmt->bindParam(':pid', $data['PID']);
      $stmt->execute();
      header('Location: show-products.php?success=Product added to currently available product!');
      }
    ...
    {
      try{
        $stmt = $conn->prepare("INSERT INTO `tblproducts`(`ProductCode`, `Description`, `UnitOfMeasure`, `UnitCost`) VALUES (:prod_code, :description, :UnitOfMeasure, :UnitCost)");
        ...
        $stmt = $conn->prepare("INSERT INTO `tblwarehouse`(`PID`, `QUANTITY`) VALUES (LAST_INSERT_ID(), :quantity)");
        $stmt->bindParam(':quantity', $quantity);
        $stmt->execute();
        header('Location: show-products.php?success=Product successfully added!');
  ...
  <div class="wrapper">
    <header class="main-header">
      <a href="../index.php" class="logo">
        ...
        <span class="logo-lg"><b>Dr. Pablo O. Torre Medical Center </b></span>
      </a>
             ...
             <div class="modal-body">
              <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>" method="post">
                <div class="form-group has-feedback">
                  <input required="" type="text" class="form-control" placeholder="Product Code" name="prodcode">
                ...
                <div class="form-group has-feedback">
                  <input required="" type="number" min="0.01" step="0.01" class="form-control" placeholder="Unit Cost" name="unitcost">
                  <span class="glyphicon glyphicon-user form-control-feedback"></span>
                </div>
                ...
                <div class="form-group has-feedback">
                  ...
                  <input required="" type="text" name="quantity" class="form-control input-number" value="1">
                  <span class="input-group-btn">
